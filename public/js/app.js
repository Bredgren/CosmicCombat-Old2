...
(function() {
  var BaseCharacter, Bindings, Camera, Characters, DebugDraw, DevGui, DevGuySmall, Energy, EnergyAttack, EnergyAttack1, Game, Goku, H, Jackie, ParticleSystem, Planet, StarField, Universe, W, b2Collision, b2Common, b2Contacts, b2Controllers, b2DebugDraw, b2Dynamics, b2Joints, b2Math, b2Shapes, b2Vec2, boundedValue, createCircle, log_input, main, settings, stance, toCapitalCoords, toLowerCoords,
    ...
    Bindings.prototype._getUSKey = function(key, layout) {
      ...
      switch (layout) {
        ...
        case Bindings.COLEMAK:
          if (!(key in Bindings.COLEMAK_MAP)) {
            return key;
          }
  ...
  EnergyAttack = (function() {
    EnergyAttack.KEY_NAMES = {
      E1: 0,
      E2: 1,
      E3: 2,
      E4: 3
    ...
    EnergyAttack.prototype.combo = {};

    function EnergyAttack(character) {
      this.character = character;
    }

    EnergyAttack.prototype.charge = function() {
      return console.log("charge energy attack");
    };

    EnergyAttack.prototype.release = function() {
      return console.log("release energy attack");
    };

    EnergyAttack.prototype.stop = function() {
      return console.log("stop energy attack");
    };

    EnergyAttack.prototype._generate_combo = function(length) {
      var combo;
  ...
  EnergyAttack1 = (function(_super) {
    __extends(EnergyAttack1, _super);

    EnergyAttack1.combo = null;

    function EnergyAttack1(character) {
      this.charge = __bind(this.charge, this);      EnergyAttack1.__super__.constructor.call(this, character);
      if (!EnergyAttack1.combo) {
        EnergyAttack1.combo = this._generate_combo(3);
      }
    }

    EnergyAttack1.prototype.combo = function() {
      return EnergyAttack1.combo;
    };

    EnergyAttack1.prototype.charge = function(time) {
      var p;

      p = this.character.body.GetPosition();
      return console.log("charge energy attack 1 (" + time + ") " + p.x);
    };

    EnergyAttack1.prototype.release = function() {
      return console.log("release energy attack 1");
    };

    EnergyAttack1.prototype.stop = function() {
      return console.log("stop energy attack 1");
    };

    return EnergyAttack1;
  ...
  BaseCharacter = (function() {
    ...
    BaseCharacter.prototype.attacks = null;
    ...
    BaseCharacter.prototype._positionSprite = function(sprite) {
      ...
      sprite.position.y = pos.y + (this._offset || 0);
  ...
  DevGuySmall = (function(_super) {
    __extends(DevGuySmall, _super);

    DevGuySmall.prototype.recover_rate = 0.0003;
    ...
    function DevGuySmall(universe, init_pos, click_callback) {
      ...
      this.attacks = [];
      this.attacks.push(new EnergyAttack1(this));
    }

    DevGuySmall.prototype.update = function() {
      return DevGuySmall.__super__.update.call(this);
    };

    DevGuySmall.prototype.draw = function() {
      return this._positionSprite(this.stand);
    };

    DevGuySmall.prototype.size = function() {
      return {
        w: this.stand.width,
        h: this.stand.height
      };
  ...
  Characters = (function() {
    ...
    Characters.DEV_GUY_SMALL = "Dev Guy Small";

    Characters.DEFAULT = "Defualt";

    Characters.TYPES = [Characters.DEFAULT, Characters.DEV_GUY_SMALL, Characters.JACKIE, Characters.GOKU];
  ...
  DevGui = (function() {
    ...
    DevGui.prototype.MOUSE_LEFT = 0;

    DevGui.prototype.MOUSE_RIGHT = 2;
    ...
    DevGui.prototype._createNewCharFolder = function() {
      ...
      f = this.root_folder.addFolder("New Character Settings");
    ...
    DevGui.prototype.onMouseDown = function(button, screen_pos) {
      var m;

      m = null;
      if (button === this.MOUSE_LEFT) {
        m = this.left_mouse;
      } else if (button === this.MOUSE_RIGHT) {
        m = this.right_mouse;
      }
      switch (m) {
        case "new character":
  ...
  Universe = (function() {
    ...
    Universe.prototype.removeTerrain = function(x, y, size, prec) {
      return this.current_planet.removeTerrain(x, y, size, prec);
    };
  ...
  Game = (function() {
    ...
    Game.prototype.onKeyDown = function(key_code) {
      if (this._controlled_char) {
        return this.key_bindings.onKeyDown(key_code);
      }
    };

    Game.prototype.onKeyUp = function(key_code) {
      if (this._controlled_char) {
        return this.key_bindings.onKeyUp(key_code);
      }
    };
    ...
    Game.prototype._onEnergyComboButtonDown = function(combo_button) {
      var attack, combo, end_time, i, same, time, _i, _j, _len, _ref, _ref1, _results;
      ...
      _ref = this._controlled_char.attacks;
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        attack = _ref[_i];
        combo = attack.combo();
        if (combo.keys.length === this.pressed_energy_combo.keys.length) {
          ...
          if (same) {
            ...
            this.pressed_energy_combo.attack = attack;
            attack.charge(time);
            break;
          } else {
    ...
    Game.prototype._onEnergyComboButtonUp = function(combo_button) {
      ...
      if (keys.length === 1 && keys[0] === this.energy_combo_key) {
        return this.pressed_energy_combo.attack.release();
      } else if (keys.length === 0) {
        if (this.pressed_energy_combo.attack) {
          this.pressed_energy_combo.attack.stop();
        }
        this.energy_combo_key = 0;
        return this.pressed_energy_combo.attack = null;
      }
    };
    ...
    Game.prototype._initKeyBindings = function() {
      ...
      this.keys.e1 = this.key_bindings.bind(settings.BINDINGS.E1, function() {
        return _this._onEnergyComboButtonDown(EnergyAttack.KEY_NAMES.E1);
      }, function() {
        return _this._onEnergyComboButtonUp(EnergyAttack.KEY_NAMES.E1);
      });
      this.keys.e2 = this.key_bindings.bind(settings.BINDINGS.E2, function() {
        return _this._onEnergyComboButtonDown(EnergyAttack.KEY_NAMES.E2);
      }, function() {
        return _this._onEnergyComboButtonUp(EnergyAttack.KEY_NAMES.E2);
      });
      this.keys.e3 = this.key_bindings.bind(settings.BINDINGS.E3, function() {
        return _this._onEnergyComboButtonDown(EnergyAttack.KEY_NAMES.E3);
      }, function() {
        return _this._onEnergyComboButtonUp(EnergyAttack.KEY_NAMES.E3);
      });
      this.keys.e4 = this.key_bindings.bind(settings.BINDINGS.E4, function() {
        return _this._onEnergyComboButtonDown(EnergyAttack.KEY_NAMES.E4);
      }, function() {
        return _this._onEnergyComboButtonUp(EnergyAttack.KEY_NAMES.E4);
      });
  ...
  $(function() {
    ...
    DOM_LOADED = true;
    assets = ["assets/img/jackie_chun.json", "assets/img/goku.json", "assets/img/planet_1.json", "assets/img/dev_guy_small.json"];
